    ...
    <div style={{
      ...
      <div style={{ padding: "1.75rem 2rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          {/* Col 1 — Objectif de revenu */}
          <div style={cardBase}>
            <SectionTitle>🎯 Objectif de revenu</SectionTitle>
            <KVRow label="Revenu net annuel actuel" value={`${fmt(revenuNetAnnuel)}/an`} valueColor="#fff" />
            <KVRow label="% du revenu visé à la retraite" value={`${pctRevenuVise} %`} valueColor={GOLD} />
            <KVRow label="Objectif en $ d'aujourd'hui" value={`${fmt(revenuDesireAuj)}/an`} valueColor={GOLD} />
            <KVRow label="Objectif en $ futurs (inflation)" value={`${fmt(revenuDesireFutur)}/an`} valueColor="#E0B44B" />
            {revenuGarantiAuj > 0 && (
              <div style={{ marginTop: 10, padding: "8px 12px", borderRadius: 9, background: "rgba(91,196,160,0.07)", border: "1px solid rgba(91,196,160,0.18)" }}>
                <p style={{ fontSize: 11, color: "#5BC4A0" }}>
                  Revenus garantis (SV + RRQ + F.P.) : <strong>{fmt(revenuGarantiAuj)}/an</strong>
                </p>
              </div>
            )}
          </div>
          ...
          {/* Col 2 — Hypothèses */}
          <div style={cardBase}>
            <SectionTitle>⚙️ Hypothèses de calcul</SectionTitle>
            <KVRow label="Âge actuel" value={`${ageActuel} ans`} />
            <KVRow label="Âge de retraite cible" value={`${ageRetraite} ans`} />
            <KVRow label="Espérance de vie" value={`${esperanceVie} ans`} />
            <KVRow label="Épargne de retraite actuelle" value={fmt(epargneActuelle)} />
            <KVRow label="Épargnes mensuelles actuelles" value={`${fmt(epargneMensActuelle)}/mois`} />
            <KVRow label="Taux d'inflation estimé" value="2,1 %" />
            <KVRow label="Rendement avant retraite" value="6,00 %" valueColor={GOLD} />
            <KVRow label="Rendement pendant retraite" value="3,00 %" valueColor={GOLD} />
          </div>
        </div>
