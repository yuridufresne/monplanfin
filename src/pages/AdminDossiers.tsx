      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <Link to="/admin/kpi" style={{
              padding: "8px 14px", borderRadius: 10, border: "1px solid rgba(201,160,99,0.35)",
              background: "rgba(201,160,99,0.1)", color: "#C9A063", fontSize: 12, fontWeight: 600,
              textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6,
            }}><FileText size={14} /> KPI</Link>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {[["ABF", "/analyse"], ["Feuille Résumé", "/resume"], ["Tableau de bord", "/dashboard"]].map(([lbl, path]) => (
                <button key={path} onClick={(e) => { e.stopPropagation(); navigate(path + "?client=" + encodeURIComponent(d.client_courriel) + "&nom=" + encodeURIComponent(d.client_nom || "") + "&retour=" + encodeURIComponent("/admin/dossiers")); }} style={{ padding: "6px 12px", borderRadius: 8, cursor: "pointer", background: "rgba(201,160,99,0.12)", border: "1px solid rgba(201,160,99,0.4)", color: "#C9A063", fontSize: 11.5, fontWeight: 600 }}>{lbl}</button>
              ))}
            </div>
          </div>
