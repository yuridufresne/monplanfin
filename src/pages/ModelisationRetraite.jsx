    ...
    <div style={{ overflowX:"auto", borderRadius:10, border:"1px solid rgba(255,255,255,.09)" }}>
      <table style={{ width:"100%", borderCollapse:"collapse", minWidth:680 }}>
        <thead>
          ...
          <tr>
            <th style={S.th}>RRQ+PSV</th><th style={S.th}>Pension</th><th style={{...S.th,borderRight:"1px solid rgba(255,255,255,.07)"}}>FERR</th>
          ...
          {vis.map((r,i)=>{
            ...
            return (
              <tr key={r.ages+i} style={{background:bg}}>
                ...
                <td style={{...S.td,color:"#5BC4A0"}}>{fmtk(r.rrqSvPension)}</td>
                <td style={{...S.td,color:"#C9A063"}}>—</td>
      ...
      {tab==="plan" && (
        <div>
          ...
          <div style={{ display:"flex", gap:12, marginBottom:14, flexWrap:"wrap", alignItems:"center", background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:12, padding:"12px 16px" }}>
            {[
              ["Rendement", <select key="rend" value={rend} onChange={e=>setRend(+e.target.value)} style={S.select}>{[3,4,5,6,7,8].map(v=><option key={v} value={v} style={{background:"#0D1628"}}>{v}%/an</option>)}</select>],
              ["Espérance de vie", <select key="esp" value={espVie} onChange={e=>setEspVie(+e.target.value)} style={S.select}>{[85,88,90,93,95,100].map(v=><option key={v} value={v} style={{background:"#0D1628"}}>{v} ans</option>)}</select>],
              ["Cible annuelle (80% brut)", <div key="cib" style={{display:"flex",alignItems:"center",gap:6}}><input type="number" value={cible} onChange={e=>setCible(+e.target.value)} style={{...S.input,width:110}} /><span style={{fontSize:11,color:"rgba(255,255,255,.35)"}}>$/an</span></div>],
            ].map(([l,ctrl])=>(
              <label key={l} style={{ display:"flex", alignItems:"center", gap:7, ...S.label }}>
                {l} <span style={{marginLeft:2}}>{ctrl}</span>
              </label>
            ))}
          </div>
              ...
              <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.08)", borderRadius:12, padding:"14px 16px", marginBottom:14 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
                  {[
                    ...
                    ["Rendement", <select key="r" value={rend} onChange={e=>setRend(+e.target.value)} style={S.select}>{[2,3,4,5,6,7,8,9,10].map(v=><option key={v} value={v} style={{background:"#0D1628"}}>{v}%/an</option>)}</select>],
